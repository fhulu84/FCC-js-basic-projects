...
form.addEventListener('submit', function(e){
  // form submits to a server, here we prevent this behaviour
  e.preventDefault();
  // in console: strings are in black colour, numbers are in blue colors
  const value = parseInt(amount.value); // all inputs are string
  ...
  if(isNaN(value) || value <= 0 || value > 9){
    ...
    result.innerHTML = tempText.map(t => `<p class="result">${t}</p>`).join('');
  }
})
